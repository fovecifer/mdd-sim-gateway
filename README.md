...
> **自用分支 / 开发预览：** 本 fork 在上游 v1.5.2 基础上增加默认关闭的 Telegram
> 双向短信，仅限一个授权用户私聊、绑定一张 SIM、逐条确认、VoWiFi 发送。
> 这是本 fork 的扩展，不是上游提供的功能。部署前请阅读
> [双向短信与安全升级说明](docs/TELEGRAM_SMS.md)，不要直接运行下方的上游在线安装命令覆盖自用版本。
...
- Telegram 默认仅用于单向通知；本 fork 可另行启用受限的、需逐条确认的双向短信，不提供远程通话或系统命令。
...
> **合规警告：** 本软件仅供号码实名持有人在运营商明确允许的范围内自用。严禁用于诈骗、群呼、营销骚扰、验证码接收、号码或线路出租、代拨转接、隐藏实际控制地点，或向第三人提供电信服务。使用者必须遵守所在地法律、电话实名制和运营商协议；本项目不构成任何电信业务许可或运营商授权。MDD Sim Gateway 最多保存和运行 **5 条 SIM 线路**，不提供独立 SIP 账号或 Telegram 远程拨号及挂断功能。本 fork 的短信扩展仅限已授权本人、单卡和逐条确认。技术限制不代表某种使用方式当然合法。
